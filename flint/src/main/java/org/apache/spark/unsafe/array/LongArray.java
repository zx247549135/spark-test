...
public final class LongArray {
  ...
  public void setLowHalf(int index, int value) {
    assert index >= 0 : "index (" + index + ") should >= 0";
    assert index < length : "index (" + index + ") should < length (" + length + ")";
    PlatformDependent.UNSAFE.putInt(baseObj, baseOffset + index * WIDTH, value);
  }

  public void setHighHalf(int index, int value) {
    assert index >= 0 : "index (" + index + ") should >= 0";
    assert index < length : "index (" + index + ") should < length (" + length + ")";
    PlatformDependent.UNSAFE.putInt(baseObj, baseOffset + index * WIDTH + 4, value);
  }

  public int getLowHalf(int index) {
    assert index >= 0 : "index (" + index + ") should >= 0";
    assert index < length : "index (" + index + ") should < length (" + length + ")";
    return PlatformDependent.UNSAFE.getInt(baseObj, baseOffset + index * WIDTH);
  }

  public int getHighHalf(int index) {
    assert index >= 0 : "index (" + index + ") should >= 0";
    assert index < length : "index (" + index + ") should < length (" + length + ")";
    return PlatformDependent.UNSAFE.getInt(baseObj, baseOffset + index * WIDTH + 4);
  }
